...
const applyModels = async () => {
  ...
  return knex.schema.hasTable(modelsTable).then(async exists => {
    if (!exists) {
      ...
      .then(async db_models => {
        ...
        for (const model in models) {
          ...
          if (db_model) {
                ...
                await knex.schema.alterTable(model, async table => {
                  ...
                  if (models[model][column].type === 'enum') {
                    ...
                    if (
                      ...
                      const removedEnums = db_model_json_column.dataset.filter(
                        db_e =>
            ...
            db_model.model_seeded = await seedModelIfSeedableAndNotSeeded(
              model,
...
createTriggerFunctions($config.database); // create trigger functions first
applyModels(); // then apply models
