...
const models = modelsDirExists
  ? require(path.join(modelsPath, './index.js'))
...
const assignHashesAndWrite = async model => {
  ...
  if (!models[model].hash) {
    // assign new model hash
    models[model].hash = Buffer.from(`[${Date.now()}]${model}`).toString(
      'base64url'
    );
  ...
  for (const column in models[model]) {
    ...
    if (!models[model][column].hash) {
      ...
      models[model][column].hash = Buffer.from(
        `[${Date.now()}]${model}.${column}`
      ).toString('base64url');
  ...
  if (newHashAssigned) {
    ...
    if (modelsDirExists) {
      ...
      writeFile(
        `${modelsPath}/${model}.js`,
        `module.exports = ${util.inspect(models[model], false, 2)}`,
        err => {
          if (err) throw err;
...
const db = {
      ...
      return knex
        ...
        .then(async db_models => {
            ...
            if (model_exists)
              for (const column in row.model_json) {
                ...
                const column_exists = Object.keys(models[model_exists]).find(
                  c =>
                    row.model_json[column].hash ===
                    models[model_exists][c].hash
                );
                ...
                if (!column_exists) {
                  ...
                  // log
                  console.log(
                    `\x1b[31m[${row.model_name}.${column} removed]\x1b[0m`
                  );
              ...
              // delete model table
              await knex.raw(
                `DROP TABLE IF EXISTS "${row.model_name}" CASCADE`
              );
          ...
          for (const model in models) {
            if (modelsDirExists) {
              // assignHashesAndWrite only works with models dir (for now)
            ...
            if (db_model) {
              ...
              for (const column in models[model]) {
                ...
                const doesColumnHashMatches = hash => {
                  const db_columnNames = Object.keys(
                    db_model.model_json
                  ).filter(c => c !== 'hash');
                  ...
                  await knex.schema.alterTable(model, async table => {
                    ...
                    if (
                      ...
                      if (
                        ...
                        if (removedConstraints.length) {
                          ...
                          console.log(
                            `\x1b[36m[${model} constraints removed: \x1b[32m${removedConstraints.join(
                              ', '
                            )}]\x1b[0m`
                          );
                        }
                    ...
                    if (
                      ...
                      if (db_model_json_column.references) {
                        ...
                        console.log(
                          `\x1b[31m[${db_model_json_column.references} foreign key \x1b[0m constraint removed from ${model}.${column}]`
                        );
                      }
                    ...
                    if (models[model][column].type === 'enum') {
                      ...
                      if (
                        ...
                        const removedEnums =
                          db_model_json_column.dataset.filter(
                            db_e =>
                              !models[model][column].dataset.find(
                                e => e === db_e
                              )
                          );
                    ...
                    ['charset', 'defaultTo', 'comment', 'unsigned'].forEach(
                      fn => {
                        if (
                          ...
                          console.log(
                            `\x1b[36m[${model}.${column}.${fn} removed]\x1b[0m`
                          );
                        } else if (
                          models[model][column][fn] &&
                          !db_model_json_column[fn]
                ...
                // not matched. create a new column
                  ...
                  await knex.schema.alterTable(model, async table => {
                    eval(
                      generateSchemaSQL(modelWithOneColumn, { debug })[model]
                    );
              ...
              db_model.model_seeded = await db.seed(
                model,
                db_model.model_seeded,
                options.noSeed
              );
            ...
            // create new database state for the model
              ...
              const seeded = await db.seed(model, false, options.noSeed);
   ...
   * @param {boolean} noSeed
   * @returns boolean
   */
  seed: async (model, seeded, noSeed) => {
    if (!seeded && !noSeed) {
   ...
   * @param {any} database
   */
  createTriggerFunctions: async database => {
    const mode = (process.env.NODE_ENV || 'development').toLowerCase();

    if (!database.triggerFunctions)
      throw new Error(
        '`database.triggerFunctions` are required for auto-timestamp, maybe other things in the future'
      );

    for (const fn in database.triggerFunctions) {
      await knex
        .raw(database.triggerFunctions[fn])
        .then(() => {
