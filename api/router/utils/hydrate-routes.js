...
export default (routes, define) => {
  const hydrate = (routes, define) => {
    Promise.all(Object.keys(routes).map(async function (from) {
      await Promise.all(routes[from].map(async route => {
        ...
        route.middleware = ['interceptor', ...route.middleware]
  ...
  const setDefaultColumns = async (route, key) => {
    ...
    if (route.columns) {
      ...
      console.error('please define a model first!')
      // throw new Error('please define a model first!')
    }
