...
const createRouter = ({ routes, define, static, plugins }, options) => {
  ...
  if (plugins && typeof plugins === 'object') {
    ...
    for (const plugin in plugins) {
      ...
      if (!plugins[plugin].base) throw Error(`Please define mavi plugins.${plugin}.base`)
      if (!plugins[plugin].routes) throw Error(`Please define mavi routes for ${plugin}`)
  ...
  // http methods:
  const methods = ['get', 'post', 'put', 'delete', 'head', 'connect', 'trace', 'patch']
        ...
        $router[route.method](route.path, ...route.middlewares.map(setMiddlewares), async (req, res) => {
          ...
          // Use default controllers
            ...
            if (route.view && !route.controller) {
              ...
              // execute controller
              return await req.app.controller(req, res)[route.controller](...$arguments)
            }
          }
          else {
